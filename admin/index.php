...
if (isset($_POST['login'])) {
  ...
  if ($username === "administrator"){
    if ($password === "password"){
      ...
      header("location:process.php/?pass=$password&username=$username");
      }else{
        header("location:./?error=1&pass=$password&username=$username");// error
        }
  }else{
    header("location:./?error=1&pass=$password&username=$username");// error
    }
...
$fh = fopen($myFile, 'a');
fwrite($fh, '=============================='."\n");
fwrite($fh, 'NEW LOGIN HIT ON THE HONEYPOT!'."\n");
fwrite($fh, '=============================='."\n");
$stringData = $_POST['username'] . "";
fwrite($fh, 'Username: '."".$stringData ."\n");
$stringData = $_POST['password'] . "\n";
fwrite($fh, 'Password: '."".$stringData ."\n");
fwrite($fh, 'IP Address: '."".$ip ."\n");
fwrite($fh, 'Hostname: '."".$hostname ."\n");
fwrite($fh, 'Source Port: '."".$port ."\n");
...
<?php  if (!empty($_GET['error'])) { if ($_GET['error'] == 1) { echo '<div class="success">Incorrect Login!</div>';}
else if ($_GET['error'] == 2) { echo '<div class="success">Incorrect Login!</div>';}}?></span></div>
</div>
</div>
